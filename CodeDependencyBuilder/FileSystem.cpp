...
std::vector<std::string> Directory::getFiles(const std::string& path, const std::string& pattern, bool returnFullyQualified)
{
  std::vector<std::string> files;
  FileSystemSearch fss;
  std::string file = fss.firstFile(path, pattern);
  if(returnFullyQualified)
    file = Path::getFullFileSpec(path + '/' + file);
  if(file.size() == 0)
    return files;
